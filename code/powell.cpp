...
#include <limits>
    ...
    struct Brent : Bracketmethod {
        ...
        double xmin, fmin;
        const double tol;
        Brent(const double toll = 0.00000003) : tol(toll) {}
    ...
    struct Powell : Linemethod<T> {
        ...
        const double ftol;
        Powell(T &func, const double ftoll = 0.000000003) : Linemethod<T>(func), ftol(ftoll) {}
        ...
        vector<double> minimise(vector<double> &pp, MatDoub_IO &ximat) {
            ...
            const int ITMAX = 1000;
            const double TINY = 0.000000000000000001;
            ...
            for (iter = 0;;++iter) {
                ...
                if ( iter == ITMAX ) cerr << "powell exceeding maximum iterations." << endl;
                for (int j = 0; j < n; j++) {
                    ptt[j] = 2.0*p[j] - pt[j];
                    xi[j] = p[j] - pt[j];
                    pt[j] = p[j];
